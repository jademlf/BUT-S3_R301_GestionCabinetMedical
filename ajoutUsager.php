...
<head>
    ...
    <script>
        // Fonction pour valider le numéro de sécurité sociale
        function validateNumSecuSociale() {
            // Récupérer la valeur du numéro de sécurité sociale depuis l'élément HTML
            var numSecuSociale = document.getElementById('numSecuSociale').value;
            // Expression régulière pour le format du numéro de sécurité sociale
            var regex = /^[1-37-8][0-9]{12}$/;
            // Vérifier si le numéro de sécurité sociale correspond au format attendu
            if (!regex.test(numSecuSociale)) {
                // Afficher une alerte si le format est invalide
                alert("Format invalide pour le numéro de sécurité sociale.");
                return false; // Empêcher la soumission du formulaire
            }
            return true; // Autoriser la soumission du formulaire si le numéro est valide
        }
    </script>
</head>
            ...
            <?php
                ...
                if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                    ...
                    if ($insertResult) {
                        ...
                        echo '<p style="color: #0097b2;">Votre usager a été ajouté avec succès.</p>';
                    } else {
