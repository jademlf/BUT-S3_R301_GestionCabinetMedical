<nav>
    <ul id="menu">
        <li><a href="index.php"><img src="/r301-projet-php/logo_projet-php.png" alt="Logo Projet PHP" width="125" height="125"></a></li>
        <li><a href="statistiques.php">Statistiques</a></li>
        <li><a href="affichageRendezVous.php">Rendez-vous</a>
            <ul>
                <li><a href="ajoutRendezVous.php">Ajout</a></li>
            </ul>
        </li>
        <li><a href="affichageMedecin.php">Médecins</a>
            <ul>
                <li><a href="ajoutMedecin.php">Ajout</a></li>
                <li><a href="rechercheMedecin.php">Recherche</a></li>
            </ul>
        </li>
        <li><a href="affichageUsager.php">Usagers</a>
            <ul>
                <li><a href="ajoutUsager.php">Ajout</a></li>
                <li><a href="rechercheUsager.php">Recherche</a></li>
            </ul>
        </li>
        <li class="deconnexion"><a href="deconnexion.php">Déconnexion ►</a></li>

    </ul>
</nav>

